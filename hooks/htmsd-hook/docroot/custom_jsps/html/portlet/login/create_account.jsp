...
<div id="confirm-otp">
	<aui:input type="text" name="confirm-otp" label="Enter OTP" value="" />
	<div id="counter"></div>
	<aui:button id="btn-confirm-otp" value="Confirm"/>
</div>
<script>
	$(function() {
		...
		function sendOTP() {
			...
			if (phoneNum == "") {
				...
				$('#confirm-otp').dialog('open');
				startTimer(60 * 2);
				Liferay.Service('/htmsd-portlet.shoppingitem/send-otp', {
					phoneNumber : phoneNum
				}, function(obj) {
					if (obj.Status == "Success") {
						currentSessionId = obj.Details;
					} else {
						currentSessionId = "";
					}
				});
		...
		function verifiyOTP() {
			var confirmOTP = $('#<portlet:namespace/>confirm-otp').val();
			if (confirmOTP == "") {
				alert("Please Enter OTP");
			} else if (currentSessionId != "") {
				Liferay.Service('/htmsd-portlet.shoppingitem/verify-otp', {
					sessionId : currentSessionId,
					otp : confirmOTP
				}, function(obj) {
					if (obj.Status == "Success" && obj.Details == "OTP Matched") {
						$('#confirm-otp').dialog('close');
						$('.span6.otp-btn').html("Verified").css({
							"color":"green",
							"font-weight":"bold"
						});
